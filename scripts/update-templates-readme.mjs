...
function readTemplateReadme(templateId) {
  ...
  if (!existsSync(path)) {
    throw new Error(
      `Template "${templateId}" is missing templates/${templateId}/README.md`,
    );
  }
...
function readdesign(readme) {
  const text = section(readme, "design");
  ...
  if (bareUrl) {
    return { label: "design", url: bareUrl[0].trim() };
  }
  return null;
}
...
if (startIndex === -1 || endIndex === -1) {
  throw new Error(
    `Could not find markers ${startMarker} and/or ${endMarker} in templates/README.md`,
  );
}
...
console.log(
  `Successfully updated templates/README.md with ${templates.length} templates.`,
);
...
console.log(
  `Successfully updated website/content/templates.json with ${websiteTemplates.length} templates.`,
);
