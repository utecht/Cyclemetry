...
The release script syncs that version to `package.json`, `app/package.json`, and `src-tauri/Cargo.lock`, commits those release metadata files, creates a `vX.Y.Z` git tag, pushes `main` plus the tag, and starts the Release workflow for all platforms with publishing enabled.
